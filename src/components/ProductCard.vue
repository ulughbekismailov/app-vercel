  ...
  >
    <div class="relative aspect-square overflow-hidden bg-gray-100 dark:bg-gray-700">
      ...
      <button
        ...
        class="absolute top-2 left-2 w-9 h-9 rounded-full flex items-center justify-center transition-all duration-200 active:scale-90"
        :class="[
          isLiked 
      ...
      >
        <svg 
          ...
          :class="{ 
            'fill-current scale-110': isLiked,
            'scale-100': !isLiked
          }"
          fill="none" 
          stroke="currentColor" 
    ...
    <div class="p-3 space-y-2">
      ...
      <div v-if="product.rating" class="flex items-center gap-1">
        <svg 
          ...
          :key="i"
          class="w-3.5 h-3.5"
          :class="i <= Math.floor(product.rating) ? 'text-yellow-400' : 'text-gray-300 dark:text-gray-600'"
          fill="currentColor"
          viewBox="0 0 20 20"
        >
...
const isLiked = computed(() => {
  return favoriteStore.isLiked(props.product.id);
});
...
// ❤️ Handle like/unlike button click
const handleLikeClick = async (event) => {
  event.stopPropagation();
  
  // Haptic feedback BEFORE the action (instant response)
  telegram.hapticFeedback('selection');
  
  try {
    // Toggle favorite with product data for optimistic update
    ...
    // Error is already handled in store with haptic feedback
...
/* ============================================================
   LINE CLAMP (Truncate text to 2 lines)
   ============================================================ */
.line-clamp-2 {
  display: -webkit-box;
  -webkit-line-clamp: 2;
